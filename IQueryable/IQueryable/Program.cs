    ...
    class Program
    {
       public struct People
        ...
        public static void Print(IOrderedEnumerable<People> arraypeople)
        {
            ...
            foreach (var item in arraypeople)
            {
                Console.WriteLine($"Имя = {item.name}--Возрост={item.age}--Дата рождения={item.Date_of_birth.ToString("d")}--Живой={item.alive}");
            }
            Console.WriteLine();
        }

        //выборка живые, по возрастанию
        public static void WithWhere(People[] Arraypeople)
        {
            var arraypeople = Arraypeople.Where(e => e.alive == true).OrderBy(e => e.age);
            Print(arraypeople);
        }

        //выборка по возросту, по убыванию
        public static void WithOrderByDescending(People[] Arraypeople)
        {
            var arraypeople = Arraypeople.Where(e => e.age > 90).OrderByDescending(e => e.age);                 
            Print(arraypeople);            
        }
        ...
        {
            Console.WriteLine($"Сумма по столб. age всех живых = {Arraypeople.Where(e => e.alive == true).Sum(u => u.age)}");
